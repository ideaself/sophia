...
async function makeEndedClass(convId: string, textbookId: string | null, tail = '傅里叶光学：上次讲到菲涅耳衍射'): Promise<void> {
  ...
  await conversationStore.addMessage(conv.id, 'user', '上课')
  await conversationStore.addMessage(conv.id, 'assistant', '好的')
  await conversationStore.endConversation(conv.id)
  await artifactStore.create(conv.id as ConversationId, 'handoff_tail', tail)
  void convId
}
...
describe('loadHandoffTail — 接力尾巴按教材隔离', () => {
  it('同教材的上一课：继承接力尾巴', async () => {
    ...
    const res = await loadHandoffTail(dataRoot, conversationStore, artifactStore, COMPANION, 'conv_new', 'tb_fourier')
  ...
  it('换了教材：绝不继承旧教材的接力尾巴（bug 复现场景）', async () => {
    ...
    await artifactStore.create(conv.id as ConversationId, 'handoff_tail', '傅里叶光学：上次讲到菲涅耳衍射')
    await writeMeta(conv.id, 'tb_fourier')
    const res = await loadHandoffTail(dataRoot, conversationStore, artifactStore, COMPANION, 'conv_calculus', 'tb_calculus')
  ...
  it('无教材课堂只继承无教材的上一课', async () => {
    ...
    await artifactStore.create(conv.id as ConversationId, 'handoff_tail', '傅里叶光学：上次讲到菲涅耳衍射')
    await writeMeta(conv.id, 'tb_fourier')
    const res = await loadHandoffTail(dataRoot, conversationStore, artifactStore, COMPANION, 'conv_free', null)
    expect(res.tail).toBeUndefined()
  })
  ...
  it('legacy 回退（无 meta 文件）：按教材过滤已下课课堂', async () => {
    ...
    const calculus = await loadHandoffTail(dataRoot, conversationStore, artifactStore, COMPANION, 'conv_calculus_new', 'tb_calculus')
    expect(calculus.tail).toBe('微积分：上次讲到导数定义')

    // 新开傅里叶课 → 拿到傅里叶的尾巴
    const fourier = await loadHandoffTail(dataRoot, conversationStore, artifactStore, COMPANION, 'conv_fourier_new', 'tb_fourier')
    expect(fourier.tail).toBe('傅里叶光学：上次讲到菲涅耳衍射')
  })

  it('当前课堂自身不参与接力（excludeConversationId）', async () => {
    await writeMeta('conv_self', 'tb_fourier')
    const res = await loadHandoffTail(dataRoot, conversationStore, artifactStore, COMPANION, 'conv_self', 'tb_fourier')
    expect(res.tail).toBeUndefined()
  })
})
